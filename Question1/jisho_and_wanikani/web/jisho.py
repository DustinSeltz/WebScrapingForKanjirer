...
class Jisho:
    ...
    @property
    @clean
    def main_meaning(self) -> list:
        res = self._soup.find("div", {"class": "kanji-details__main-meanings"})
        self._main_meaning = res.getText().split(", ")
        return self._main_meaning
    ...
    def variants(self) -> list:
        parent = self._soup.find("dl", {"class": "dictionary_entry variants"})
        child = parent.findChildren("dd")
        self._variants = [s.getText() for s in child]
    ...
    def data(self):
        for keyword in self._keyword:
            ...
            cell.update({"main_meaning": self.main_meaning})
            # cell.update({"variants": self.variants})
            yield cell
